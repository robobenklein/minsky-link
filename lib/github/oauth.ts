...
export class GitHubOauth {
  ...
  constructor(
    opts: Github.Options,
    width = 800,
    height = 800 //, node_integration=false)
  ) {
  ...
  private set_token(error: any, response: any, body: any): void {
    ...
    if (response.statusCode >= 400) {
      throw new Error(
        "HTML Error:" + response.statusCode + response.statusMessage
      );
    }
    var token_start = 13;
  ...
  private get_token(returncode: string): void {
    ...
    request.post(
      ...
      (error: any, response: any, body: any) =>
        this.set_token(error, response, body)
    );
    return;
  }
  ...
  public validate(): Promise<boolean> {
    ...
    request.get(
      ...
      (error: any, response: any, body: any) =>
        this.set_validated(error, response, body)
    );
